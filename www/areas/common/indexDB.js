...
// 此处可以创建多个表
      ...
      get: function (articleId, objectStoreName, success, fail) {
        //console.log(articleId);
        db.open(function () {
          var
            store = db.getObjectStore(objectStoreName),
            req = store.get(articleId);
...
// delete是保留字
      'delete': function (articleId, objectStoreName, success, fail) {
        db.open(function () {
          var
            mode = 'readwrite',
            store, req;
          store = db.getObjectStore(objectStoreName, mode);
          req = store.delete(articleId);
          req.onsuccess = success;
          req.onerror = fail;
        });
      },
